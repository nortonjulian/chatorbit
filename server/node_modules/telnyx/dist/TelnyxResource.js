...
TelnyxResource.prototype = {
    ...
    createFullPath(commandPath, urlData) {
        return path
            .join(this.basePath(urlData), this.path(urlData), typeof commandPath == 'function' ? commandPath(urlData) : commandPath)
    ...
    createResourcePathWithSymbols(pathWithSymbols) {
        return ('/' +
            path.join(this.resourcePath, pathWithSymbols || '').replace(/\\/g, '/')); // ugly workaround for Windows
    },
    ...
    _timeoutHandler: function (timeout, req, callback) {
        return () => {
            ...
            req._isAborted = true;
            req.abort();
            callback.call(this, new TelnyxError.TelnyxConnectionError({
        ...
        return function (res) {
            ...
            res.on('end', function () {
                ...
                try {
                    ...
                    return callback.call(self, new TelnyxError.TelnyxAPIError({
                ...
                callback.call(self, null, responseBody);
            });
        };
    },
    ...
    _generateConnectionErrorMessage(requestRetries) {
        return ('An error occurred with our connection to Telnyx.' +
            (requestRetries > 0
    ...
    _request: function (method, _host, path, data, auth, options, callback) {
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const self = this;
        let requestData;
        ...
        function retryRequest(requestFn, headers, requestRetries) {
            requestRetries += 1;
            return setTimeout(requestFn, self._getSleepTimeInMS(requestRetries), headers, requestRetries);
        }
        function makeRequest(headers, numRetries) {
            const timeout = self._telnyx.getApiField('timeout');
            const isInsecureConnection = self._telnyx.getApiField('protocol') == 'http';
