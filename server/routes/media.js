...
media.get('/chatrooms/:id/media', verifyToken, async (req, res) => {
  ...
  const rows = await prisma.messageAttachment.findMany({
    ...
    select: {
      id: true, kind: true, url: true, mimeType: true, width: true, height: true,
      durationSec: true, caption: true, createdAt: true,
      message: { select: { id: true, createdAt: true, sender: { select: { id: true, username: true, avatarUrl: true }}}}
    }
  });
