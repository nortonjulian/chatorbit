...
export function randomBytes(n=32){ return crypto.randomBytes(n); }
export function toB64(buf){ return Buffer.from(buf).toString('base64'); }
export function fromB64(b64){ return Buffer.from(b64, 'base64'); }

export function hkdf(keyMaterial, info='provision-v1', len=32){
  return crypto.hkdfSync('sha256', Buffer.alloc(0), Buffer.from(keyMaterial), Buffer.from(info), len);
}
