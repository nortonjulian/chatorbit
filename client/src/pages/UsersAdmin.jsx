...
export default function UsersAdminPage() {
  ...
  const setFlags = async (id, patch) => {
    await axiosClient.patch(`/admin/users/${id}/flags`, patch);
    fetchUsers();
  };
  const ban = async (id) => {
    await axiosClient.post(`/admin/users/${id}/ban`);
    fetchUsers();
  };
  const unban = async (id) => {
    await axiosClient.post(`/admin/users/${id}/unban`);
    fetchUsers();
  };
        ...
        <Table.Tbody>
          {items.map((u) => (
            <Table.Tr key={u.id}>
              ...
              <Table.Td>
                {u.isBanned ? (
                  <Button size="xs" variant="light" onClick={() => unban(u.id)}>
                    Unban
                  </Button>
                ) : (
                  <Button
