...
import { Modal, Stack, Group, Button, NumberInput, Select, TextInput, Image, CopyButton, Tooltip } from '@mantine/core';
...
export default function RoomInviteModal({ opened, onClose, roomId }) {
  ...
  useEffect(() => { if (!opened) { setInvite(null); setQr(null); } }, [opened]);
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Invite people" centered>
      <Stack gap="md">
        ...
        <Button onClick={createInvite} loading={loading}>Generate link</Button>
              ...
              <CopyButton value={invite.url} timeout={1500}>
                {({ copied, copy }) => (
                  <Tooltip label={copied ? 'Copied!' : 'Copy'}>
                    <Button variant="light" onClick={copy}>{copied ? 'Copied' : 'Copy link'}</Button>
                  </Tooltip>
                )}
              </CopyButton>
              <Button variant="light" onClick={() => window.open(invite.url, '_blank')}>Open</Button>
            </Group>
