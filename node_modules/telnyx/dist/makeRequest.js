...
function _getRequestOpts(self, requestArgs, spec, overrideData) {
    ...
    const commandPath = typeof spec.path == 'function'
        ? spec.path
    ...
    for (let i = 0, l = urlParams.length; i < l; ++i) {
        ...
        if (param == 'id' && typeof arg !== 'string') {
            ...
            throw new Error('Telnyx: "id" must be a string, but got: ' +
                typeof arg +
                ' (on API request to `' +
                requestMethod +
                ' ' +
        ...
        if (!arg) {
            ...
            path = self.createResourcePathWithSymbols(spec.path);
            throw new Error('Telnyx: Argument "' +
                urlParams[i] +
                '" required, but got: ' +
                arg +
                ' (on API request to `' +
    ...
    if (args.length) {
        ...
        throw new Error('Telnyx: Unknown arguments (' +
            args +
    ...
    return new Promise(function (resolve, reject) {
        ...
        function requestCallback(err, response) {
            if (err) {
                ...
                resolve(spec.transformResponseData && response?.data
